...
if (missingKeys.length > 0) {
  ...
  console.error(
    "[Firebase] Available env vars:",
    Object.keys(process.env).filter((k) => k.startsWith("EXPO")),
  );
  console.error(
    "[Firebase] Constants.expoConfig?.extra:",
  ...
  console.error(
    "[Firebase] Config validation failed - app may not work correctly",
  );
}
...
try {
  ...
  if (error.code === "auth/already-initialized") {
    ...
    console.warn(
      "[Firebase] ⚠️ Could not use AsyncStorage persistence, using default getAuth:",
      error.message,
    );
    auth = getAuth(app);
...
// Apply Identity Platform tenant scope. Must be set before any sign-in call;
// tokens issued without it will not be accepted by the new GCP backend.
const tenantId =
  Constants.expoConfig?.extra?.authTenantId ||
  process.env.EXPO_PUBLIC_AUTH_TENANT_ID;
if (tenantId) {
  auth.tenantId = tenantId;
  console.log("[Firebase] ✅ Auth tenant set:", tenantId);
} else {
  console.warn(
    "[Firebase] ⚠️ No EXPO_PUBLIC_AUTH_TENANT_ID set — auth will use the default (non-tenant) user pool.",
  );
}

// Initialize Firestore
const databaseId =
  Constants.expoConfig?.extra?.firebaseDatabaseId ||
