...
function isNoInternetError(normalized: string): boolean {
  const hasNetworkToken =
    normalized.includes("network") ||
    normalized.includes("internet") ||
    normalized.includes("offline") ||
    normalized.includes("failed to fetch");
  if (hasNetworkToken) return true;
  const hasTimeout =
  ...
  return hasTimeout && hasNetworkContext;
}
...
export function getFriendlyErrorCopy(
  ...
  if (context === "login") {
    ...
    if (normalized.includes("organization")) {
      return {
        title: "Login Error",
        message:
          "Your account is not assigned correctly. Please contact your manager.",
        category: "login_failed",
      };
    }
  ...
  if (isPermissionError(normalized)) {
    if (context === "recording" || context === "camera") {
      return {
  ...
  if (context === "camera") {
    return {
      ...
      message:
        "We couldn't switch to the external camera. Make sure it's plugged in and you've granted USB access, then try again.",
