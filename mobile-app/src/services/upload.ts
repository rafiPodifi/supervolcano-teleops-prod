...
function buildResumableUploadStartUrl(
  bucket: string,
  storagePath: string,
): string {
...
async function getFirebaseStorageAuthHeader(): Promise<string> {
  ...
  if (!token) {
    throw new Error(
      "No authenticated user available for Firebase Storage upload",
    );
  }
...
async function startResumableUploadSession(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to start Firebase resumable upload (${response.status}): ${responseText}`,
    );
  }
  ...
  if (!uploadUrl) {
    throw new Error(
      "Firebase resumable upload session did not return X-Goog-Upload-URL",
    );
  }
...
async function uploadFileWithNativeTask(
  ...
  if (result.status < 200 || result.status >= 300) {
    throw new Error(
      `Firebase upload failed (${result.status}): ${result.body}`,
    );
  }
}
...
export async function getVideoMetadata(
  videoUri: string,
): Promise<{ durationSeconds: number; fileSize: number }> {
  try {
...
export async function uploadVideoToFirebase(
  ...
  try {
    ...
    // Job-less (location-only) uploads land under an "unassigned" segment.
    const storagePath = `media/${locationId}/${jobId ?? "unassigned"}/${fileName}`;
    ...
    emitStage(
      onStage,
      "start_resumable",
      "Starting Firebase resumable upload session",
      storagePath,
    );
    onProgress({
      bytesTransferred: 0,
    ...
    await uploadFileWithNativeTask(
      uploadUrl,
      normalizedVideoUri,
      fileSize,
      contentType,
