...
- App shows a pending upload even after all videos have been uploaded. Eventually resolved itself. FIXED — root cause was FileSystem.deleteAsync (50–500ms) running between the final persistState notification and removeFromQueue. Reordered so the queue removal happens FIRST (instant badge update) and file deletion runs fire-and-forget in the background.
- I have noticed twice that an upload failed. I believe it was because the phone went to sleep(?). MITIGATED — added expo-keep-awake activation while uploads are in flight, deactivated when queue idle. Covers screen-lock during active foreground uploads. Full background protection (Android Doze on fully-closed app) still needs a foreground service — tracked as future work.
